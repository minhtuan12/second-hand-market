import {ProfileOutlined, ShopOutlined, ShoppingOutlined} from "@ant-design/icons";
...
const FeatureItem = ({item}: { item: IFeature }) => {
    return <Link href={item.link} className={'text-[14px] text-black hover:text-[#f80]'}>
        <Flex vertical align={'center'} justify={'center'} gap={5} className={'p-3 hover:bg-slate-100 rounded-lg'}>
            {item.icon}
...
export default function PopoverFeature() {
    ...
    return <div className={'bg-[white] rounded-[12px] shadow-sm p-5'}>
        {
            eachRow.map((items: IFeature[], index: number) => (
                <Row key={index} gutter={[8, 24]}>
                    {
                        items.map((item: IFeature) => (
                            <Col key={item.link} xs={24} lg={12} xxl={8} xl={8} className={'min-w-[100px]'}>
