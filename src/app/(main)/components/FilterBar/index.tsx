...
import {Checkbox, Flex, Radio, Slider} from "antd";
import '../../../global.scss'
...
export default function FilterBar({categories}: { categories: Category[] }) {
    ...
    const handleSelectPriceFilter = (isFree: boolean) => {
        ...
        dispatch(setFilter({
            ...filter,
            priceFrom: isFree ? null : 1000,
            priceTo: isFree ? null : 50000000
        }))
        setPrice({
            from: isFree ? null : 1000,
    ...
    return <>
        <div className={'h-[calc(100%_-_388px)]'}>
            ...
            <Checkbox.Group
                className={'flex flex-col gap-1 scrollbar-thin overflow-hidden hover:overflow-auto h-[calc(100%_-_38px)] custom-checkbox'}
                style={{scrollbarGutter: 'stable'}}
                onChange={e => {
                    const params = new URLSearchParams(searchParams)
                    if (e?.length > 0) {
                        dispatch(setFilter({...filter, categoryIds: e}))
                        params.delete('categoryIds')
                        e?.forEach((id: any) => params.append('categoryIds', id))
                    } else {
                        dispatch(setFilter({...filter, categoryIds: null}))
                        params.delete('categoryIds')
                    }
                    router.push(`?${params.toString()}`)
                }}
                value={filter.categoryIds}
                options={categories?.map((item: Category) => ({label: item?.name, value: item?._id}))}
            />
        </div>
        <div>
            <div className={'text-[17px] mb-2 flex justify-between items-center'}>
                Theo giá sản phẩm
                <div className={'cursor-pointer text-[#f80]'} onClick={() => {
                    setResetPrice(true)
                    dispatch(setFilter({
                        ...filter,
                        priceFrom: null,
            ...
            <Radio.Group value={resetPrice ? null : isPriceless} className={'w-full justify-between'}>
                <Radio onClick={() => handleSelectPriceFilter(true)} value={true} className={'!text-base'}>Đồ cho
                    tặng</Radio>
                <Radio onClick={() => handleSelectPriceFilter(false)} value={false} className={'!text-base'}>Đồ trả
                    phí</Radio>
            </Radio.Group>
            {
        ...
        <div>
            <div className={'text-[17px] mb-2'}>Theo tình trạng sản phẩm</div>
            <Checkbox.Group value={filter.condition} onChange={e => {
                dispatch(setFilter({...filter, condition: e?.length > 0 ? e : null}))
                const params = new URLSearchParams(searchParams)
            ...
            }}>
                {
                    Object.values(PRODUCT_CONDITION).map((condition: any) => (
                        <Checkbox key={condition.LABEL} value={condition.VALUE} className={'!text-base'}>
                            {condition.LABEL}
                        </Checkbox>
                    ))
                }
